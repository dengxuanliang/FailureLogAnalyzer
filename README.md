...
- **前端已提供 Operations 上传/任务监控页面**；同时也保留 API/curl 上传路径，便于脚本化接入。
...
本地 Compose 主要读取**仓库根目录** shell 环境或 **仓库根目录 `.env`** 文件中的变量；**不是** `backend/.env`。
...
> **安全提示**
>
> 上述默认账号仅适合 `localhost` / 演示环境。  
> 如果要在局域网、共享主机或任何非本机环境运行，请务必覆盖 `SECRET_KEY` 与演示账号密码。
>
...
1. **前端 Operations 页面（推荐交互演示）**
   - 登录后打开 `http://localhost:3000/operations`
   - 上传 JSONL 文件
   - 观察 ingest job 状态与完成结果
2. **API / curl（推荐脚本化接入）**
   - 适合 CI、自动化 smoke、批量导入
...
如果你要做**交付前 / CI / 更严格的验收**，推荐使用严格模式：

```bash
SMOKE_ALLOW_DEMO_FALLBACK=0 SMOKE_REQUIRE_REPORT_DONE=1 ./scripts/smoke-main-flow.sh
```
...
- `SMOKE_ALLOW_DEMO_FALLBACK`（默认 `1`；upload ingest 不可用时回退到 `dev-seed-demo` 路径。**验收时建议设为 `0`**）
- `SMOKE_REQUIRE_REPORT_DONE`（默认 `0`；设为 `1` 时要求 report 最终状态为 `done`。**验收时建议设为 `1`**）
